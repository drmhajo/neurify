...
export default function GeneralAnnouncementScreen() {
  ...
  const publish = () => {
    if (!title.trim() || !message.trim() || !approvalSecret.trim()) {
    ...
    Alert.alert(language === "en" ? "Send department-wide announcement?" : "إرسال إشعار عام للقسم؟", language === "en" ? `This will appear in the notification inbox for ${recipientCount} active users and be pushed to their registered devices.` : `سيظهر هذا التنبيه في صندوق الإشعارات لدى ${recipientCount} مستخدمين نشطين، ويرسل كتنبيه فوري إلى أجهزتهم المسجلة.`, [
      ...
      { text: language === "en" ? "Send announcement" : "إرسال الإشعار", onPress: () => { void (async () => {
        setSending(true);
        const result = await sendGeneralAnnouncement({ title, message, approvalSecret });
        setSending(false);
        if (!result.ok) {
        ...
        setApprovalSecret("");
        Alert.alert(language === "en" ? "Announcement sent" : "تم إرسال الإشعار", language === "en" ? `The announcement was added for ${result.recipientCount} active users. The stable Push request was submitted to ${result.pushSubmitted} registered device(s).` : `تمت إضافة الإشعار لـ ${result.recipientCount} مستخدمين نشطين، وأُرسل الطلب عبر الاتصال الآمن إلى ${result.pushSubmitted} جهازًا مسجلًا.`);
        router.back();
      })(); } },
    ]);
  };
