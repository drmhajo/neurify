...
export default function GeneralAnnouncementScreen() {
  ...
  const publish = () => {
    ...
    Alert.alert(language === "en" ? "Send department-wide announcement?" : "إرسال إشعار عام للقسم؟", language === "en" ? `This will appear in the notification inbox for ${recipientCount} active users.` : `سيظهر هذا التنبيه في صندوق الإشعارات لدى ${recipientCount} مستخدمين نشطين.`, [
      ...
      { text: language === "en" ? "Send announcement" : "إرسال الإشعار", onPress: () => {
        ...
        Alert.alert(language === "en" ? "Announcement sent" : "تم إرسال الإشعار", language === "en" ? `The announcement was added for ${result.recipientCount} active users.` : `تمت إضافة الإشعار لـ ${result.recipientCount} مستخدمين نشطين.`);
        router.back();
      } },
    ]);
  };
