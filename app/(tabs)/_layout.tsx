...
export default function TabLayout() {
  ...
  return <RequireSession><Tabs screenOptions={{ headerShown: false, tabBarActiveTintColor: palette.navy, tabBarInactiveTintColor: "#829AB1", tabBarButton: HapticTab, tabBarStyle: { height: 60 + bottomPadding, paddingTop: 8, paddingBottom: bottomPadding, backgroundColor: "#FFFFFF", borderTopColor: palette.line }, tabBarLabelStyle: { fontSize: 10, fontWeight: "700" } }}>
    ...
    <Tabs.Screen name="reports" options={{ title: "التقارير", tabBarIcon: ({ color }) => <IconSymbol size={24} name="doc.text.fill" color={color} /> }} />
    <Tabs.Screen name="schedule" options={{ title: "الجداول", tabBarIcon: ({ color }) => <IconSymbol size={24} name="calendar" color={color} /> }} />
    <Tabs.Screen name="teams" options={{ title: "الفرق", tabBarIcon: ({ color }) => <IconSymbol size={24} name="person.3.fill" color={color} /> }} />
    <Tabs.Screen name="admin" options={{ title: "الإدارة", href: session?.role === "admin" ? undefined : null, tabBarIcon: ({ color }) => <IconSymbol size={24} name="gearshape.fill" color={color} /> }} />
  </Tabs></RequireSession>;
}
