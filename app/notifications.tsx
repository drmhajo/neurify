...
export default function NotificationsScreen() {
  ...
  const openNotification = (item: TeamNotification) => {
    ...
    else if (item.type === "report_request" || item.type === "report_request_reminder") router.push("/(tabs)/reports");
  ...
  const renderItem = ({ item }: { item: TeamNotification }) => {
    ...
    const isReportRequest = item.type === "report_request" || item.type === "report_request_reminder";
    const icon = isGeneral ? "campaign" : isReport ? "assignment" : isReportRequest ? "assignment-late" : isConsultation ? "forum" : "hotel";
    const color = isGeneral ? palette.gold : isReport ? palette.navy : isReportRequest ? "#B42318" : isConsultation ? palette.teal : palette.gold;
    const background = isGeneral ? "#FFF3D4" : isReport ? palette.paleBlue : isReportRequest ? "#FFF0EE" : isConsultation ? palette.paleTeal : "#FFF3D4";
    const typeLabel = isGeneral ? (language === "en" ? "General announcement" : "إشعار عام") : isReport ? (language === "en" ? "Shift report" : "تقرير مناوبة") : isReportRequest ? (language === "en" ? "Report request" : "طلب تقرير") : isConsultation ? (language === "en" ? "Consultation" : "استشارة") : (language === "en" ? "Inpatient" : "حالة منوّمة");
    const source = isGeneral || isReport ? (language === "en" ? "Neurosurgery Department" : "قسم جراحة المخ والأعصاب") : item.teamName;
    const action = isGeneral ? (language === "en" ? "Read announcement" : "قراءة الإشعار") : isReport ? (language === "en" ? "Open report" : "فتح التقرير") : isReportRequest ? (language === "en" ? "Open report requests" : "فتح طلبات التقارير") : localize("فتح غرفة الفريق");
