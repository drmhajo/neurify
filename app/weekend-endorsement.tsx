...
export default function WeekendEndorsementScreen() {
  ...
  return <View style={styles.page}>
    ...
    <AppCard style={styles.summary}><View style={[styles.summaryRow, direction(isRTL)]}><View style={styles.summaryIcon}><MaterialIcons name="weekend" size={24} color="#FFFFFF" /></View><View style={styles.flex}><Text style={[styles.summaryTitle, align(isRTL)]}>{language === "en" ? "Weekend handover report" : "تقرير تسليم نهاية الأسبوع"}</Text><Text style={[styles.summaryText, align(isRTL)]}>{language === "en" ? `${report.entries.length} inpatient records for ${scopeLabel}.` : `يتضمن التقرير ${report.entries.length} من المرضى المنومين للاستشاري: ${scopeLabel}.`}</Text></View></View>
      ...
      {canExport ? <View style={styles.exportPanel}><View style={[styles.exportPanelHead, direction(isRTL)]}><View style={styles.flex}><Text style={[styles.exportTitle, align(isRTL)]}>{language === "en" ? "Export filtered report" : "تصدير التقرير المصفّى"}</Text><Text style={[styles.exportHint, align(isRTL)]}>{language === "en" ? "Save as PDF or spreadsheet" : "احفظ كـ PDF أو جدول بيانات"}</Text></View><View style={styles.exportBadge}><MaterialIcons name="file-download" size={19} color={palette.navy} /></View></View><View style={[styles.exportActionsProminent, direction(isRTL)]}><ExportAction icon="picture-as-pdf" label={language === "en" ? "Export PDF" : "تصدير PDF"} accessibilityLabel={language === "en" ? "Export filtered report as PDF" : "تصدير التقرير المصفّى PDF"} tone="pdf" isRTL={isRTL} disabled={busy || !report.entries.length} onPress={() => { void exportReport("pdf"); }} /><ExportAction icon="grid-on" label={language === "en" ? "Export Excel" : "تصدير Excel"} accessibilityLabel={language === "en" ? "Export filtered report as Excel" : "تصدير التقرير المصفّى Excel"} tone="excel" isRTL={isRTL} disabled={busy || !report.entries.length} onPress={() => { void exportReport("xlsx"); }} /></View></View> : <Text style={[styles.permission, align(isRTL)]}>{language === "en" ? "Report management permission is required to export." : "يتطلب التصدير صلاحية إدارة التقارير."}</Text>}
    </AppCard>
...
function ExportAction({ icon, label, accessibilityLabel, tone, isRTL, disabled, onPress }: { icon: "picture-as-pdf" | "grid-on"; label: string; accessibilityLabel: string; tone: "pdf" | "excel"; isRTL: boolean; disabled: boolean; onPress: () => void }) { return <Pressable disabled={disabled} accessibilityRole="button" accessibilityLabel={accessibilityLabel} onPress={onPress} style={({ pressed }) => [styles.exportActionProminent, tone === "pdf" ? styles.pdfExportAction : styles.excelExportAction, disabled && styles.exportActionDisabled, pressed && !disabled && styles.pressed]}><MaterialIcons name={icon} size={19} color="#FFFFFF" /><Text style={[styles.exportActionTextProminent, { writingDirection: isRTL ? "rtl" : "ltr" }]}>{label}</Text></Pressable>; }
