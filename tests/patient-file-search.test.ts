...
import { normalizeFileNumber, normalizePatientSearchText, searchActivePatientFiles } from "../lib/patient-file-search";
...
describe("البحث برقم الملف أو التشخيص", () => {
  ...
  it("يعثر على الحالات النشطة بالتشخيص دون تغيير مطابقة رقم الملف", () => {
    expect(normalizePatientSearchText("  DIAGNOSIS  ")).toBe("diagnosis");
    expect(searchActivePatientFiles(data, "diagnosis")[0]).toMatchObject({ id: "case-1", diagnosis: "Diagnosis" });
    expect(searchActivePatientFiles(data, "not-a-diagnosis")).toEqual([]);
  });
});
