...
describe("AI medical-report draft workflow", () => {
  it("protects generation behind central-session validation and rate limiting", () => {
    ...
    expect(route).toContain("generateMedicalReportDraftWithGemini");
  ...
  it("uses Gemini server-side to generate the reviewed report template from minimized data only", () => {
    const route = source("server/medical-report-draft.ts");
    ...
    expect(refinement).toContain("Use only facts explicitly documented in the supplied clinicalData object");
    expect(refinement).toContain("Do not add, remove, infer, reinterpret, summarize beyond clear linguistic organization, or change any clinical fact");
    ...
    expect(route).toContain('generationEngine: "gemini"');
  });

  it("uses Gemini server-side to generate the reviewed report template from minimized data only", () => {
    const route = source("server/medical-report-draft.ts");
    const generator = source("server/gemini-medical-report-refinement.ts");
    expect(route).toContain("generateMedicalReportDraftWithGemini");
    expect(route).toContain("hasValidDepartmentSession(accountId, dataProof)");
    expect(route).not.toContain('model: "gpt-5-mini"');
    expect(generator).toContain("GEMINI_API_KEY");
    expect(generator).toContain("gemini-3.6-flash");
    expect(generator).toContain("Use only facts explicitly documented in the supplied clinicalData object");
    expect(generator).toContain("Do not add, remove, infer, reinterpret, summarize beyond clear linguistic organization, or change any clinical fact");
    expect(generator).toContain("Preserve every numeric value exactly");
    expect(generator).not.toContain("fullName");
    expect(generator).not.toContain("fileNumber");
    expect(generator).not.toContain("ward");
    expect(generator).not.toContain("bed");
    expect(route).toContain('generationEngine: "gemini"');
  });
});
