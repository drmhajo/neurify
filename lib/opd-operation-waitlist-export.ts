...
export type OpdWaitlistExportResult = "shared" | "downloaded" | "print-opened" | "unavailable";
...
export async function exportOpdWaitlistExcel(report: OpdWaitlistReport, language: OpdWaitlistExportLanguage): Promise<OpdWaitlistExportResult> {
  const fileName = opdWaitlistExportFileName(report, "xlsx");
  const base64 = XLSX.write(createOpdWaitlistWorkbook(report, language), { bookType: "xlsx", type: "base64", compression: true });
