...
export function buildWeekendEndorsementReport(data: DepartmentData, generatedBy: string): WeekendEndorsementReport {
  return {
    generatedAt: new Date().toISOString(),
    generatedBy,
    entries: data.teams.flatMap((team) => team.cases
      .filter((patient) => patient.status === "منوّم")
      .map((patient) => ({
        id: patient.id,
