...
export async function exportApprovedMedicalReport(input: { patient: { fullName: string; fileNumber: string; age: number | null; diagnosis: string; ward?: string; bed?: string }; draft: MedicalReportDraft; language: "ar" | "en"; approvedBy: string; template?: MedicalReportPrintTemplate }) {
  ...
  const labels = medicalReportSectionLabels(outputLanguage);
  const arabic = outputLanguage === "ar";
  ...
  const formalEnglishStyle = formalEnglish ? ".formal-document-label{letter-spacing:1.1px;text-transform:uppercase;font-size:8px;color:#168B7A;font-weight:700;margin:0 0 4px}.formal-patient-title{font-size:11px;text-transform:uppercase;letter-spacing:.5px;color:#4956A6;margin:15px 0 0}.formal-attestation{border:1px solid #C5E8E0;border-radius:10px;padding:10px 12px;background:#F3FBF9;color:#235E55;font-size:9px;line-height:1.5;margin:12px 0}.formal-signoff{display:grid;grid-template-columns:1fr 1fr;gap:18px;margin-top:24px}.formal-signature{border-top:1px solid #9CA7BC;padding-top:7px;color:#29364B;font-size:9px}.formal-signature span{display:block;color:#6D778C;font-size:8px;margin-top:3px}" : "";
  const reportTitle = formalEnglish ? "Patient Medical Report" : (arabic ? "مسودة تقرير طبي معتمدة" : "Approved Medical Report Draft");
  const subtitle = formalEnglish ? "Official clinical documentation — reviewed prior to issue" : (arabic ? "تمت مراجعة المحتوى واعتماده من الطبيب قبل التصدير." : "Content reviewed and approved by a clinician before export.");
  const introduction = formalEnglish ? "Confidential clinical document. This report is prepared from documented patient-file information and has been reviewed by the approving clinician." : (arabic ? "تنبيه: هذه الوثيقة مولدة كمساعدة في التوثيق من البيانات المسجلة في الملف، وقد راجعها المستخدم المعتمد قبل التصدير." : "Notice: This document was AI-assisted from documented file data and reviewed by the approved user before export.");
  const signoff = formalEnglish ? `<div class="formal-signoff"><div class="formal-signature">${escapeHtml(input.approvedBy)}<span>Reviewing clinician</span></div><div class="formal-signature">${date}<span>Issue date and time — Riyadh</span></div></div>` : "";
