...
export function MedicalReportDraftAction({ patient, session, language, isRTL }: { patient: PatientCase; session: DepartmentSession | null; language: "ar" | "en"; isRTL: boolean }) {
  ...
  const generate = async () => {
    ...
    try {
      ...
      setReviewNotice(result.reviewNotice);
      setApproved(false);
      setExportedPdf(null);
      setOpen(true);
    } catch (error) {
    ...
    <Modal visible={open} animationType="slide" onRequestClose={() => setOpen(false)}>
      <View style={styles.modalPage}>
        ...
        <ScrollView contentContainerStyle={styles.content}>
          ...
          {reviewNotice ? <Text style={[styles.reviewNotice, align(isRTL)]}>{reviewNotice}</Text> : null}
