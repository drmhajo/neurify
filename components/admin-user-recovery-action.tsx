...
type Props = {
  ...
  resetPassword: (userId: string) => Promise<{ ok: boolean; temporaryPassword?: string }>;
...
export function AdminUserRecoveryAction({ visible, users, currentUserId, resetPassword, removeUser, isRTL, language }: Props) {
  ...
  const reset = async (user: DepartmentUser) => {
    ...
    setBusyId(null);
    Alert.alert(result.ok ? (language === "en" ? "Temporary password issued" : "تم إصدار كلمة مرور مؤقتة") : (language === "en" ? "Password reset blocked" : "تم رفض استعادة كلمة المرور"), result.ok ? (language === "en" ? `Give this temporary password to ${user.name} through an approved channel: ${result.temporaryPassword}. They must change it after sign-in.` : `سلّم كلمة المرور المؤقتة للمستخدم ${user.name} عبر قناة معتمدة: ${result.temporaryPassword}. ويجب عليه تغييرها بعد تسجيل الدخول.`) : (language === "en" ? "Only an active department administrator can reset this account." : "يمكن لمشرف القسم النشط فقط استعادة هذا الحساب."));
  };
